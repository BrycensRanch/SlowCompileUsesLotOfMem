...
public class WindowsAPI : NativeAPI
{
    ...
    {
        var gch = GCHandle.FromIntPtr((IntPtr)lParam);
        if (!gch.IsAllocated || gch.Target is not List<WindowInfo> list)
            return false;
        ...
        PInvoke.GetWindowText(hwnd, new PWSTR(buffer), 256);
    ...
    public override unsafe List<WindowInfo> GetWindowList()
    {
        ...
        try
        {
            ...
            PInvoke.EnumWindows(callback, (LPARAM)GCHandle.ToIntPtr(handle));
        }
        finally
        {
    ...
    private static readonly string ShellExtMenuName = "SnapX";
    ...
    private static readonly string ShellExtDesc = "Upload with SnapX";
    ...
    private static readonly string ShellExtEditDesc = "Edit with SnapX";
    ...
    {
        return CheckShortcut(Environment.SpecialFolder.SendTo, "SnapX", Assembly.GetEntryAssembly()!.Location);
    }

    [RequiresAssemblyFiles()]
    public static bool CreateSendToMenuButton(bool create)
    {
        return SetShortcut(create, Environment.SpecialFolder.SendTo, "SnapX",
